...
namespace WithoutVisitor
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            double TotalCost = CalculateInPieces(soldInPieces)+ CalculateInWeight(soldInWeight);
        ...
        private static double CalculateInPieces(List<ItemSoldInPieces> soldInPieces)
        {
            double total = 0.0D;
            foreach (var item in soldInPieces)
            {
                total = total + item.UnitPrice * item.NumberOfPieces;
            }
            return total;
        }

        private static double CalculateInWeight(List<ItemSoldInWeight> soldInWeight)
        {
            double total = 0.0D;
            foreach (var item in soldInWeight)
            {
                total = total + item.UnitPrice * item.Weight;
            }
            return total;
        }
    }
